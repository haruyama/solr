...
public final class ECNaviTokenizer extends Tokenizer {
    ...
    private static SignTable getSignTable() {
        return SignTable.getInstance(
                Config.getInstance().get("allowed_sign", "+_#"));
    }
     ...
     * @param reader  リーダー
     */
    public ECNaviTokenizer(final AttributeFactory factory,
            final Reader reader) {
        super(factory);
        init(reader);
    }
    ...
    private static class TokenCandidate {
        ...
        private Token createUnhyphenatedToken() {
            Token token = new Token(charBuffer.array(),
                    charBuffer.position(), start, end,
                    TokenType.UNHYPHENATED.name());
            return token;
        }
    ...
    /**
     * キューから次のハイフン抜きTokenを取り出す.
     * 残りTokenが0になったらハイフンの状態をリセット.
     *
    ...
    private void setAttribute(final Token token) {
        setAttribute(token.getBuffer(), token.getLength(),
                token.getStart(), token.getEnd(), token.getType());
    }

    /**
     * 要素から属性を設定する.
     * @param buffer 文字列buffer
     * @param length 長さ
     * @param start  開始位置
     * @param end    終了位置
     * @param type   種類
     */
    private void setAttribute(final char[] buffer,
            final int length, final int start,
            final int end, final String type) {
    ...
    @Override
        public boolean incrementToken() throws IOException {
            ...
            /*
               ...
               */
            setAttribute(buffer, length, start,
                    start + length + concatCharCount + hankakuOffset,
                    tokenType.name());
    ...
    public boolean equals(final Object obj) {
        if (this == obj) {
            ...
            return false;
        } else if (!(obj instanceof ECNaviTokenizer)) {
            return false;
        }
        ECNaviTokenizer other = (ECNaviTokenizer) obj;
        if (!Arrays.equals(buffer, other.buffer)) {
            return false;
        } else if (hankakuOffset != other.hankakuOffset) {
            return false;
        }
        if (hyphenState == null) {
            if (other.hyphenState != null) {
                return false;
            }
        } else if (!hyphenState.equals(other.hyphenState)) {
            return false;
        }

        if (lastCharType != other.lastCharType) {
            return false;
        }
        ...
        if (pbinput == null) {
            if (other.pbinput != null) {
                return false;
            }
        } else if (!pbinput.equals(other.pbinput)) {
            return false;
        }
        if (termAtt == null) {
        ...
        if (tokenCandidates == null) {
            if (other.tokenCandidates != null) {
                return false;
            }
        } else if (!tokenCandidates.equals(other.tokenCandidates)) {
            return false;
        }
        if (typeAtt == null) {
        ...
        if (unhyphenatedQueue == null) {
            if (other.unhyphenatedQueue != null) {
                return false;
            }
        } else if (!unhyphenatedQueue.equals(other.unhyphenatedQueue)) {
            return false;
        }
        return true;
    }
    ...
    @Override
        public int hashCode() {
            ...
            result = prime * result + hankakuOffset;
            result = prime * result
                + ((hyphenState == null) ? 0 : hyphenState.hashCode());
            result = prime * result + lastCharType;
            result = prime * result + offset;
            result = prime * result
                + ((offsetAtt == null) ? 0 : offsetAtt.hashCode());
            result = prime * result + ((pbinput == null) ? 0 : pbinput.hashCode());
            result = prime * result + ((termAtt == null) ? 0 : termAtt.hashCode());
            result = prime * result
                + ((tokenCandidates == null) ? 0 : tokenCandidates.hashCode());
            result = prime * result + ((typeAtt == null) ? 0 : typeAtt.hashCode());
            result = prime
                * result
                + ((unhyphenatedQueue == null) ? 0 : unhyphenatedQueue
                        .hashCode());
            return result;
        }
}
